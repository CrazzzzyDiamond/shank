        ...
        <div className="flex flex-col items-center gap-3 sm:gap-6">
          <div className="flex items-center gap-2">
            <p className={`text-2xl font-semibold tracking-widest transition-colors duration-300 ${success ? 'text-green-400' : 'text-(--color-text-muted)'}`}>
              {showNoteLabel ? `${note.label} / ${midiToSolfege(note.midi)}` : '···'}
            </p>
            <button
              onClick={() => setShowNoteLabel(v => !v)}
              className="text-(--color-text-muted) opacity-40 hover:opacity-80 transition-opacity"
              title={showNoteLabel ? 'Сховати назву' : 'Показати назву'}
            >
              {showNoteLabel ? (
